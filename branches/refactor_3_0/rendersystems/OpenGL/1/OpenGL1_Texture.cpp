...
namespace april
{
	OpenGL1_Texture::OpenGL1_Texture(chstr filename) : OpenGL_Texture()
	{
	...
	OpenGL1_Texture::OpenGL1_Texture(int w, int h, unsigned char* rgba) : OpenGL_Texture()
	{
	...
	OpenGL1_Texture::OpenGL1_Texture(int w, int h, Format format, Type type, Color color) : OpenGL_Texture()
	{
	...
	void OpenGL1_Texture::_setCurrentTexture()
	{
		OpenGL1_RenderSystem* rendersys = (OpenGL1_RenderSystem*)april::rendersys;
	...
	bool OpenGL1_Texture::isLoaded()
	{
		return (this->textureId != 0);
	}

	bool OpenGL1_Texture::load()
	{
		if (this->textureId != 0)
		{
			return true;
		...
		// write texels
		glBindTexture(GL_TEXTURE_2D, this->textureId);
		((OpenGL1_RenderSystem*)april::rendersys)->state.textureId = ((OpenGL1_RenderSystem*)april::rendersys)->deviceState.textureId = this->textureId;
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		if (image != NULL)
		{
	...
	void OpenGL1_Texture::setPixel(int x, int y, Color color)
	{
	...
	void OpenGL1_Texture::fillRect(int x, int y, int w, int h, Color color)
	{
	...
	void OpenGL1_Texture::blit(int x, int y, Texture* texture, int sx, int sy, int sw, int sh, unsigned char alpha)
	{
		OpenGL1_Texture* source = (OpenGL1_Texture*)texture;
		x = hclamp(x, 0, this->width - 1);
		y = hclamp(y, 0, this->height - 1);
	...
	void OpenGL1_Texture::blit(int x, int y, unsigned char* data, int dataWidth, int dataHeight, int dataBpp, int sx, int sy, int sw, int sh, unsigned char alpha)
	{
		x = hclamp(x, 0, this->width - 1);
		y = hclamp(y, 0, this->height - 1);
	...
	void OpenGL1_Texture::write(int x, int y, unsigned char* data, int dataWidth, int dataHeight, int dataBpp)
	{
		x = hclamp(x, 0, this->width - 1);
		y = hclamp(y, 0, this->height - 1);
	...
	void OpenGL1_Texture::stretchBlit(int x, int y, int w, int h, Texture* texture, int sx, int sy, int sw, int sh, unsigned char alpha)
	{
		// TODO
	}

	void OpenGL1_Texture::stretchBlit(int x, int y, int w, int h, unsigned char* data, int dataWidth, int dataHeight, int dataBpp, int sx, int sy, int sw, int sh, unsigned char alpha)
	{
		// TODO
	}

	void OpenGL1_Texture::rotateHue(float degrees)
	{
		// TODO
	}

	void OpenGL1_Texture::saturate(float factor)
	{
		// TODO
	}
