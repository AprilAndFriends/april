...
namespace april
{
	...
	LRESULT CALLBACK Win32_Window::processCallback(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		case 0x0119: // WM_GESTURE (Win7+ only)
			...
			else if (wParam == 2) // GID_END
			{
				...
				{ 
					...
					april::window->queueMouseEvent(AMOUSEEVT_UP, april::window->getCursorPosition(), AK_DOUBLETAP);
				}
				_touchDown = false;
			}
			...
			{
				_doubleTapDown = true;
				april::window->queueMouseEvent(AMOUSEEVT_DOWN, april::window->getCursorPosition(), AK_DOUBLETAP);
			}
			break;
		...
		case WM_KEYDOWN:
			...
			{
				april::window->queueKeyEvent(AKEYEVT_DOWN, (april::Key)wParam, 0);
			}
			return 0;
		case WM_SYSKEYUP:
			if (wParam == VK_MENU)
			...
			// no break here, because this is still an input message that needs to be processed normally
		case WM_KEYUP:
			april::window->queueKeyEvent(AKEYEVT_UP, (april::Key)wParam, 0);
			return 0;
		case WM_CHAR:
			april::window->queueKeyEvent(AKEYEVT_DOWN, AK_NONE, wParam);
			break;
		case WM_LBUTTONDOWN:
			_touchDown = true;
			_mouseMoveMessagesCount = 0;
			april::window->queueMouseEvent(AMOUSEEVT_DOWN, april::window->getCursorPosition(), AK_LBUTTON);
		...
		case WM_RBUTTONDOWN:
			_touchDown = true;
			_mouseMoveMessagesCount = 0;
			april::window->queueMouseEvent(AMOUSEEVT_DOWN, april::window->getCursorPosition(), AK_RBUTTON);
			if (!april::window->isFullscreen())
			{
				SetCapture((HWND)april::window->getBackendId());
			}
		...
		case WM_LBUTTONUP:
			_touchDown = false;
			april::window->queueMouseEvent(AMOUSEEVT_UP, april::window->getCursorPosition(), AK_LBUTTON);
			if (!april::window->isFullscreen())
			{
				ReleaseCapture();
			}
		...
		case WM_RBUTTONUP:
			_touchDown = false;
			april::window->queueMouseEvent(AMOUSEEVT_UP, april::window->getCursorPosition(), AK_RBUTTON);
			if (!april::window->isFullscreen())
			{
				ReleaseCapture();
			}
		...
		case WM_MOUSEMOVE:
			...
			april::window->queueMouseEvent(AMOUSEEVT_MOVE, april::window->getCursorPosition(), AK_NONE);
			break;
		case WM_MOUSEWHEEL:
			_wheelDelta = (float)GET_WHEEL_DELTA_WPARAM(wParam) / WHEEL_DELTA;
			if ((GET_KEYSTATE_WPARAM(wParam) & MK_CONTROL) != MK_CONTROL)
			{
				april::window->queueMouseEvent(AMOUSEEVT_SCROLL, gvec2(0.0f, -(float)_wheelDelta), AK_NONE);
			}
			else
			{
				april::window->queueMouseEvent(AMOUSEEVT_SCROLL, gvec2(-(float)_wheelDelta, 0.0f), AK_NONE);
			}
			break;
		case WM_MOUSEHWHEEL:
			_wheelDelta = (float)GET_WHEEL_DELTA_WPARAM(wParam) / WHEEL_DELTA;
